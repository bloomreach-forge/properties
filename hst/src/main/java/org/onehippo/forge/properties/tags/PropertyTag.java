...
public class PropertyTag extends HstTagSupport {
	...
	protected int doEndTag(final HstRequest hstRequest, final HstResponse hstResponse) {
		...
		final PropertiesBean propertiesBean = language == null ? //split
				propertiesManager.getPropertiesBean(documentPath, siteContentBaseBean, hstRequest.getLocale()) : //split 
				propertiesManager.getPropertiesBean(documentPath, siteContentBaseBean, language);
